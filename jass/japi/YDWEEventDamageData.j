...
library YDWEEventDamageData
	...
	*/
	...
	function EXGetEventDamageData takes integer edd_type returns integer
		call RemoveSaveDirectory("run jass.YDWE_lua_Integer = japi.EXGetEventDamageData(" + I2S(edd_type) + ")")
		return YDWE_lua_Integer
	endfunction

	function EXSetEventDamage takes real amount returns boolean
		call RemoveSaveDirectory("run jass.YDWE_lua_Boolean = japi.EXSetEventDamage(" + R2S(amount) + ")")
