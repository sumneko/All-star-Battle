	...
	function cmd.hero_model(p, n, id, ...)
		local n		= tonumber(n)
		local hero 	= p:getHero()
		local sid	= string2id(id)
		local texts	= {}
		local jc	= p:getRecord '节操'
		local arg	= {...}
		--解析售价
		for i, s in ipairs(arg) do
			local n, gold = s:match '(%d+)%=(%d+)'
			if n then
				n, gold		= tonumber(n), tonumber(gold)
				arg[i] 		= {n, gold}
				if gold > jc then
					texts[i] = ('|cffff1111%02d 次使用权 - %04d 点节操|r'):format(n, gold)
				else
					texts[i] = ('|cff11ff11%02d 次使用权 - %04d 点节操|r'):format(n, gold)
				end
				
			else
				cmd.maid_chat(p, '皮肤参数错误,请检查:' .. s)
			end
		end

		--查询皮肤英雄
		local hero_id	= slk.ability[id].DataA1
		--皮肤昵称
		local hero_title	= slk.unit[hero_id].EditorSuffix
		--皮肤说明
		local hero_direct	= slk.unit[hero_id].Ubertip
		--皮肤称谓
		local hero_name		= slk.unit[hero_id].Propernames:match '([^%,]+)'

		--生成技能标题
		local title		= ('%s %s'):format(hero_title, hero_name)

		--生成技能说明
		local direct	= ('%s\n\n%s\n\n|cffffcc00您目前拥有 %d 点节操'):format(hero_direct, table.concat(texts, '\n'), jc)

		--添加皮肤技能
		jass.UnitAddAbility(hero, record.hero_skill[n])
		local ab	= japi.EXGetUnitAbility(hero, record.hero_skill[n])
		if p == player.self then
			japi.EXSetAbilityDataString(ab, 1, 215, title)
			japi.EXSetAbilityDataString(ab, 1, 218, direct)
			japi.EXSetAbilityDataString(ab, 1, 204, slk.unit[hero_id].Art)
		end
	end